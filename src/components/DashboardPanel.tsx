...
export default function DashboardPanel({ config }: { config: ConfigData }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <Card title="봇 가동 현황">
        <div className="space-y-4">
          <CardContent
            ...
            rightElement={<span className="inline-flex h-3 w-3 rounded-full bg-green-500 animate-pulse" />}
          />
      ...
      >
        <div className="space-y-4">
          <CardContent icon={Activity} label="트래킹 포지션" value={`${config.topN} 개`} />
          <CardContent icon={DollarSign} label="카피 금액" value={`$${config.copyAmountUsd}`} />
          <CardContent icon={Zap} label="최소 APR 트리거" value={`${config.minAprPercent}%`} />
          ...
          <div className="bg-muted/40 p-4 rounded-2xl flex flex-col gap-2 border border-border/50">
            ...
            <div className="flex flex-wrap gap-1.5 mt-1">
              {config.pools && config.pools.length > 0 ? (
                ...
                <span className="text-xs text-muted-foreground/60">전체 풀 대상 트래킹 중</span>
              )}
            </div>
          </div>
          ...
          <div className="bg-muted/40 p-4 rounded-2xl flex flex-col gap-2 border border-border/50">
            ...
            <div className="flex flex-wrap gap-1.5 mt-1">
              {config.autoRechargeTokens && config.autoRechargeTokens.length > 0 ? (
                ...
                <span className="text-xs text-muted-foreground/60">등록된 토큰 없음</span>
              )}
            </div>
          </div>
        </div>
